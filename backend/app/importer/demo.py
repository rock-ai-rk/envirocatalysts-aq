...
def write_demo_dataset(directory: Path, seed: int = 7, hourly_only: bool = False) -> None:
    """Write the synthetic dataset. `hourly_only` writes just the demo stations and their hours
    (scope "hourly"), to load on top of real city data that has no station data of its own."""
    rng = random.Random(seed)
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "manifest.json").write_text(
        json.dumps(
            {
                "name": "synthetic-demo-hourly" if hourly_only else "synthetic-demo",
                "source": "Generated by app.importer.demo for development. Not real measurements.",
                "synthetic": True,
                "scope": "hourly" if hourly_only else "all",
            },
            indent=2,
        )
    )
    if not hourly_only:
        _write_city_files(rng, directory)
    _write_station_files(rng, directory)


def _write_city_files(rng: random.Random, directory: Path) -> None:
...
def _write_station_files(rng: random.Random, directory: Path) -> None:
    with _csv(
