...
| Check | How | Result |
|---|---|---|
| Colour contrast (text 4.5:1, UI parts 3:1) | `npm run check:contrast` ([`mobile/scripts/check-contrast.mjs`](../mobile/scripts/check-contrast.mjs)), which checks every text/background pairing in the light and dark palettes, the label colour on every AQI category and pollutant colour, and — for 1.4.11 — every AQI segment against the card it is drawn on, counting its outline where the fill alone is under 3:1 | 75 / 75 pairings pass (16 Sep; 61 before the fourteen graphical-object pairings were added, 59 on 15 Sep) |
| Motion (WCAG 2.2.2, 2.3.3) | Code review of every animation in the app: the skeletons' pulse, the trend scrub, the rows' entrance and the press feedback | The three that run on their own (skeleton pulse, row entrance, press scale) are all behind the OS Reduce Motion setting — reanimated's `useReducedMotion()` or `ReduceMotion.System`. Animations move opacity and transform only, so none of them changes layout. |
